...
#   scripts/submit_team_external.py
# 将结果回显到终端，并把输入/输出/命令写入 logs/queue 便于排查

set -euo pipefail
ROOT_DIR="$(cd "$(dirname "$0")/.." && pwd)"
LOG_DIR="$ROOT_DIR/logs/queue"
SCRIPT_PY="$ROOT_DIR/scripts/submit_team_external.py"
...
usage() {
  cat <<USAGE
用法：
  $(basename "$0") [--team-config 路径] [--topic-id ID] [--mode note|search|qa] [--timeout 秒]

示例：
  $(basename "$0") --team-config config/teams/qa.deepseek_reasoner.json --topic-id t1 --mode qa --timeout 60
...
printf "\n== 提交会话开始 ==\n脚本: %s\nPython: %s\nTopic: %s\nMode: %s\nTimeout: %ss\nTeam : %s\n\n" \
  "$SCRIPT_PY" "$PY_BIN" "$TOPIC_ID" "$MODE" "$TIMEOUT" "${TEAM_CONFIG:-(无)}"
...
while true; do
  ...
  OUT_FILE="$LOG_DIR/submit_cli.last_render.md"
  CMD_FILE="$LOG_DIR/submit_cli.last_cmd.txt"
  IN_FILE_COPY="$LOG_DIR/submit_cli.last_in.md"
